...
export default function Publications() {
  return (
    <Section id="publications" number="06" title="Research & Publications" subtitle="Research & Papers">
      <div className="flex flex-col gap-8 max-w-4xl mx-auto">
        {resumeData.publications.map((item, index) => (
          // @ts-ignore - handling mixed types from resumeData
          <PublicationCard key={index} item={item} index={index} />
        ))}
      </div>
    </Section>
  );
...
function PublicationCard({ item, index }: { item: any, index: number }) {
  return (
    ...
    >
      ...
      {item.doi && (
        <a 
          href={`https://doi.org/${item.doi.replace("DOI: ", "")}`}
          target="_blank" 
